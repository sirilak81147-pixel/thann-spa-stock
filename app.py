...
def login():
    if request.method=="POST":
        ...
        c=db(); user=c.execute("SELECT * FROM users WHERE username=? AND password=?",(u,p)).fetchone(); c.close()
        if user:
            session["user"]=user["username"]; session["role"]=user["role"]; return redirect(url_for("dashboard"))
...
def dashboard():
    ...
    out=c.execute("SELECT COUNT(*) AS count FROM stock WHERE branch=? AND qty=0",(branch,)).fetchone()["count"]
    c.close()
...
def movement():
    ...
    c.execute("""INSERT INTO movements(created_at,username,branch,product_id,action,qty,before_qty,after_qty)
                 VALUES(?,?,?,?,?,?,?,?)""",(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),session["user"],branch,p["id"],action,qty,before,after))
    c.commit();c.close()
...
def history():
    c=db()
    rows=c.execute("""SELECT m.*,p.code,p.name FROM movements m JOIN products p ON p.id=m.product_id
                      ORDER BY m.id DESC LIMIT 500""").fetchall()
